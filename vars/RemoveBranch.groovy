    ...
    node("master") {
        ...
        stage("Check that codebase branch is not in use") {
            ...
            if (cdPipelines.size() != 0) {
                println "Found ${context.codebaseType} \"${context.codebase}\" branch \"${context.codebaseBranch}\" usage in CD pipelines:"
        ...
        stage("Remove custom resources") {
            new OpenshiftResource(context.codebaseBranchCR, "${context.codebase}-${context.codebaseBranch}", this).remove()
        }
        ...
        stage("Remove image stream") {
            if (context.codebaseType != "autotests")
                new OpenshiftResource("imagestream", "${context.codebase}-${context.codebaseBranch}", this).remove()
        }
        stage("Remove builds") {
            if (context.codebaseType != "autotests")
                new OpenshiftResource("bc", "${context.codebase}-${context.codebaseBranch}", this).remove()
        }
        stage("Remove sonar project") {
            if (params.REMOVE_BRANCH_SONAR_PROJECT && context.codebaseType != "autotests")
                context.sonar.removeProject("${context.codebase}:${context.codebaseBranch}")
        }
    }
}
